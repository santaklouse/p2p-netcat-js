...
| LAN discovery | mDNS |
| Internet discovery | bootstrap + IPFS Amino DHT |
| Служебные протоколы | identify и ping |

По умолчанию CLI слушает IPv4 и IPv6 на QUIC и TCP. Значение `-p 0` означает,
что операционная система выберет свободные транспортные порты. `-4`, `-6` и
`--no-quic` изменяют набор listen-адресов.
...
3. Иначе сначала проверяется локальный peer store. Он мог получить адрес через
   mDNS, bootstrap, identify или предыдущий запрос.
...
В Worker также работают Noise, Yamux, identify, ping, bootstrap discovery и
Amino DHT в client mode.
...
3. Worker загружает статический `network-config.json`. При недоступности файла
   используется встроенная конфигурация.
4. Для каждого delegated routing endpoint запрашиваются оба ресурса:
   `peers/{PeerId}` и `providers/{PeerId-CID}`. Эти HTTP-запросы выполняются
   параллельно, каждый имеет таймаут 8 секунд.
5. Если delegated routing не вернул подходящих адресов, запускается Amino DHT:
   сначала provider lookup, затем `findPeer`, с повтором до 20 секунд.
6. Из результатов удаляются адреса, которые браузер не может набрать. Сейчас
   остаются WebTransport и WS/WSS; HTTPS-страница принимает только защищённый
   WSS. TCP и обычный QUIC браузеру недоступны.
7. К адресам discovery добавляются необязательные relay из статической
   конфигурации.
8. Для каждого кандидата одновременно вызывается `dialProtocol()`. Первый
   успешный поток побеждает, остальные попытки отменяются через AbortController.
9. Победивший multiaddr сохраняется в IndexedDB и первым проверяется в
   следующей сессии.
...
EOF stdin закрывает пишущую сторону потока после необязательной задержки `-q`.
